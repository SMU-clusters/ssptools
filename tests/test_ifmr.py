...
class TestPredictors:
    ...
    @pytest.mark.parametrize(
        'FeH, expected',
        zip(metals, [[16.65645, 13.684405, 22.59955, 31.588195, 40.5],
                     [16.65645, 13.684405, 22.59955, 31.588195, 40.5],
                     [16.69869, 14.109375, 22.34194, 31.470415, 40.5],
                     [15.51396, 13.158875, 20.38378, 29.53558, 39.67126],
                     [14.66664, 13.642055, 16.65486, 23.562575, 27.53808],
                     [15.41827, 14.269495, 27.51753, 29.70356, 31.06735],
                     [15.41827, 14.269495, 27.51753, 29.70356, 31.06735]])
    )
    def test_Ba20_r_ppsn_BH_predictor(self, FeH, expected):

        mi = np.linspace(19., 100., 5)

        pred, *_ = ifmr._Ba20_r_ppsn_BH_predictor(FeH)

        assert pred(mi) == pytest.approx(expected)

    @pytest.mark.parametrize(
        'FeH, expected',
        zip(metals, [[5.95672, 13.684405, 22.59955, 31.588195, 40.5],
                     [5.95672, 13.684405, 22.59955, 31.588195, 40.5],
                     [5.75891, 14.109375, 22.34194, 31.470415, 40.5],
                     [4.7856, 13.163405, 20.38378, 29.53558, 39.67126],
                     [4.22753, 13.642055, 16.65486, 23.562575, 27.53808],
                     [2.9936, 17.61182, 27.51753, 29.70356, 31.06735],
                     [2.9936, 17.61182, 27.51753, 29.70356, 31.06735]])
    )
    def test_Ba20_d_ppsn_BH_predictor(self, FeH, expected):

        mi = np.linspace(19., 100., 5)

        pred, *_ = ifmr._Ba20_d_ppsn_BH_predictor(FeH)
